...
### Day 5: Folder Cleanup

Data structures class covered how real C++ projects are laid out: headers in
`include/`, source in `src/`, and `build/` as a throwaway folder you generate
and never commit. Mine had everything dumped in the root. Moved the headers
(Board.h, Move.h, MoveGenerator.h) into `include/` and the `.cpp` files into
`src/` with `git mv` so the history follows them. Updated `CMakeLists.txt` to
point at the new paths and to add `include/` to the compiler's search path, so
the `#include "Board.h"` lines didn't have to change. Also widened `.gitignore`
(build dirs, `*.exe`, editor cruft) and stopped tracking the `.vscode/` folder
since those files are auto-generated by the editor, not real project code.
Wiped `build/`, reconfigured from scratch, and confirmed it still compiles and
prints the same 9 moves as before. Next step is still legal move filtering.